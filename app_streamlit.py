...
def gen_pred(n_starting_points, n_steps, temperature_token, model):
...
def main_section():
    ...
    st.markdown('This is a music generation project where we try to generate classical music with an RNN. Model was trained on the parsed **MIDI** '
                'files and generated pieces are also saved in this format. One can choose from 2 different models. Frist model named **Tokens** '
                'generates tokens which are combinations of a note, duration and an offset. Second model named **Separate models** consists of '
                '3 trained models to predict notes, durations and offsets separately. Generation is done by selecting number of random '
    ...
    model_type = st.sidebar.selectbox('Select type of the model', ['Tokens', 'Separate models'])
